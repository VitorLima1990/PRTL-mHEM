...
class System:
    ...
    def DE(self, CFO, TD, GFD, V_TopInsulator, V_MiddleInsulator, V_BottomInsulator, t, tmax):        
        ...
        for V_Insulator in V_insulators:
            ...
            #Reference: F. Silveira, S. Visacro (2020) - Lightning Parameters of a Tropical Region for Engineering Application: 
            #Statistics of 51 Flashes Measured at Morro do Cachimbo and Expressions for Peak Current Distributions
            probability = 1 / (1 + (VoltageFactor * CurrentPeak / 43.3) ** 3.8)            
        ...
        #Reference: CIGRE Working Group 01 (Lightning) (1991) 
        #Brochure 63 - Guide to Procedures for Estimating the Lightning Performance of Transmission Lines
        if GFD == None:
            GFD = 0.04 * TD**1.25
        N_L = GFD * (28*(self.Structure.Ht)**0.6 + self.Structure.b) / 10
        BackFlashoverRate = 0.6 * N_L * Probs[index]        
        
        print("Top Insulator Backflashover Outages/100 km/year Rate = " + str(round(0.6 * N_L * Probs[0], 3)))
        print("Middle Insulator Backflashover Outages/100 km/year Rate = " + str(round(0.6 * N_L * Probs[1], 3)))
        print("Bottom Insulator Backflashover Outages/100 km/year Rate = " + str(round(0.6 * N_L * Probs[2], 3)))      
                
        return (round(BackFlashoverRate, 3), V_insulators[index])
