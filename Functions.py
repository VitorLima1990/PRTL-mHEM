...
class BasicInputData:
    ...
    def __init__(self,_fileLines):
        ...
        try:
            ...
            try:                
                self.constantGroundZ = float(self.fileLines[i].replace("CONSTANT GROUND RESISTANCE VALUE (INSERT -1 FOR A NEW GROUND RESISTANCE CALCULATION OR LEAVE IT BLANK FOR FREQUENCY DEPENDENT GROUND MODEL): ", ""))
            except:                
            ...
            i += 1
            try:
                self.ThunderDays = float(self.fileLines[i].replace("THUNDER DAYS IN A YEAR: ", ""))
            except:
                self.ThunderDays = None
